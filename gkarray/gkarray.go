...
// The expected usage is that Add() is called in the agent (collector), while
// Merge() and Quantile() are called in the backend; in other words, values
// are added to the sketch in the agent, sketches are sent to the backend where
// they are merged with other sketches, and quantile queries are made to the merged
...
func (s GKArray) Copy() GKArray {
	n := NewGKArray()
	if len(s.Entries) > 0 {
		n.Entries = make([]Entry, len(s.Entries), len(s.Entries))
		copy(n.Entries, s.Entries)
	}
	n.Incoming = n.Incoming[:len(s.Incoming)]
	copy(n.Incoming, s.Incoming)
	n.Min = s.Min
	n.Max = s.Max
	n.Count = s.Count
	n.Sum = s.Sum
...
func (s GKArray) Quantile(q float64) float64 {
	if q < 0 || q > 1 {
		log.Fatal("Quantile out of bounds")
		return math.NaN()
	}
